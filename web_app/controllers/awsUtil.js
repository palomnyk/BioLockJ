...
exports.listAwsProfiles = function (req, res, next) {
  try {
    // let aws = spawn(`source ${path.join(BLJ, "file")} ; launchEC2Node.sh ${req.body.formData.launchAction} ${req.body.formData.ec2StackName} ${req.body.formData.instanceName} ${req.body.formData.instanceType} ${req.body.formData.scriptName}`, {shell: '/bin/bash'});
  } catch (error) {
...
exports.listS3Buckets = function (req, res, next) {
  try {
    let aws = spawn(`source ${path.join(BLJ, 'script', 'aws_functions')} ; list_s3_buckets`, {shell: '/bin/bash'});
